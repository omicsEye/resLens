...
# Remove non-ARGs
train_df = train_df[~train_df.Class.isin(['non_ARG'])]
test_df = test_df[~test_df.Class.isin(['non_ARG'])]
...
print(f'tokenized test data. size {len(tokenized_datasets_test)}')
...
def get_training_args(fold):
    return TrainingArguments(
        f"checkpoints/{model_name}-multi_finetuned-{dataset_name}-fold{fold}",
...
for fold, (train_idx, val_idx) in enumerate(kf.split(train_sequences)):
    ...
    model = AutoModelForSequenceClassification.from_pretrained(f"{model_id}",config=config, ignore_mismatched_sizes=True,trust_remote_code=True)
    model.classifier = nn.Linear(config.hidden_size, num_labels).to(device)
...
output_file = os.path.join(output_dir, f"{model_name}_multi_results_{dataset_name}.csv")
results_df = pd.DataFrame(results)
results_df.to_csv(output_file, index=False)

# Save the best model
model_save_path = os.path.join(output_dir, f"{model_name}_multi_best_model")
...
output_file = os.path.join(output_dir, f"{model_name}_multi_test_{dataset_name}_preds.csv")
